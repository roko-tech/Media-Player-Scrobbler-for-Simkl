...
- **Local Analytics:** Review totals, type distribution, filters, and watch activity without sending history to a server.
...
5. Click any card for details and episode history. Local file paths are redacted from the viewer by default.
...
- The viewer bundles its scripts and fonts and blocks automatic network access with a restrictive Content Security Policy.
- Provider links can leave the local page only after you click them.
- Poster artwork is not fetched automatically.
- History retention is unlimited by default. A positive `history_retention_limit` setting opts into a maximum entry count.
- Set `viewer_include_file_paths` to `true` only if you explicitly want local paths copied into `data.js`.
...
### How It Works
- All scrobbled media (movies, TV, anime) are added/updated in `watch_history.json`; entries are not silently truncated under the default configuration.
